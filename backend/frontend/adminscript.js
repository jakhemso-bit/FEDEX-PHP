document.addEventListener("DOMContentLoaded", function () {
  ...
  window.saveShipment = async function () {
    ...
    const shipmentData = {
      ...
      // NEW HISTORY ENTRY
      historyEntry: (historyLocation.value && historyStatus.value) ? {
        date: new Date().toLocaleString(),
        location: historyLocation.value,
    ...
    try {
      const res = await fetch(exists ? `${API}/${shipmentData.trackingNumber}` : API, {
      ...
      // Clear only the history inputs (main form stays for convenience)
